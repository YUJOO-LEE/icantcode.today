...
**Hosting: GitHub Pages.** Not Cloudflare Pages, not Netlify, not Vercel.
The presence of `scripts/inject-cf-token.mjs` is for Cloudflare *Web
Analytics* beacon injection only — it has nothing to do with the
hosting platform.

- Domain pinned via `public/CNAME` (copied to `dist/CNAME` by Vite).
- All routes in `PRERENDER_ROUTES` (`src/constants/routes.ts`) are
  prerendered to static `dist/<route>/index.html` files by
  `scripts/prerender.mjs`. GitHub Pages serves them through directory
  index matching (`/game` → `/game/index.html`).
- SPA fallback for unknown URLs: `scripts/copy-404.mjs` copies the
  fully-injected `dist/index.html` to `dist/404.html` at the very end
  of the build. GitHub Pages then serves that file (status 404) and
  the SPA's catch-all route (`* → Navigate to /`) redirects users to
  the home page client-side.
- `_redirects` (Cloudflare/Netlify format) is **not** used. Do not
  reintroduce it; GitHub Pages ignores that file.
- `dist/sitemap.xml` is generated by `scripts/prerender.mjs` from
  `PRERENDER_ROUTES`. Adding a new prerendered route automatically
  refreshes the sitemap on next build — do not maintain it by hand.
- Build pipeline order:
  `tsc -b` → `vite build` (client) → `vite build --ssr src/entry-server.tsx --outDir dist-server`
  → `node scripts/prerender.mjs` (prerender HTML + sitemap, deletes
  `dist-server/`) → `node scripts/inject-cf-token.mjs` (all
  `dist/**/index.html`) → `node scripts/inject-gtm.mjs` (same glob)
