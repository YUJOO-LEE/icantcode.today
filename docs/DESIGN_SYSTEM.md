...
/* <html class="dark"> 으로 제어, Tailwind v4 dark: variant 사용 */
.dark {
  --color-bg: #121519;
  --color-surface: #1E2329;
  --color-border: #2D3440;
  --color-text-primary: #E8E8E8;
  --color-text-secondary: #8B9299;
...
| 심볼 | 의미 | 사용처 |
|------|------|--------|
| `>` | 일반 입력 프롬프트 | 게시물 작성, 사용자 액션 |
| `$` | 사용자 명령 | 활성 세션 사용자 표시 |
| `#` | 관리자/시스템 | 시스템 메시지, 공지 |
| `~` | 홈 디렉토리 | 경로 표현 (`@user ~/feed`) |
...
닉네임 설정 시 헤더에 현재 세션 정보 표시:

```
닉네임 설정됨:    $ @my_nickname [session]
닉네임 미설정:    $ anonymous [set nickname]   ← 클릭 시 닉네임 입력 프롬프트
```
...
```
[  > run command  ]   기본 (Primary)
[  > run command  ]   호버 시 배경색 변경
[  > loading...▌  ]   로딩 상태 (커서 깜빡임)
...
| 변형 | 스타일 |
|------|--------|
| `primary` | 테두리 + Primary 컬러 텍스트 |
| `danger` | 테두리 + Error 컬러 텍스트 |
| `ghost` | 테두리 없음, 텍스트만 |
...
<TerminalCard>
  <TerminalCard.Header>
    <TerminalPrompt user="username" path="~/feed" time="5m ago" />
  </TerminalCard.Header>
...
| 화면 크기 | 레이아웃 |
|----------|---------|
| ~768px | 단일 컬럼. 피드 카드 풀 너비 |
| 768px~ | 최대 너비 700px, 중앙 정렬 |
| 1024px~ | 최대 너비 760px, 사이드 여백 증가 |
...
| 상태 점 깜빡임 | StatusIndicator (DOWN) | 1.5s 루프 |
